...
class CashControllerTest extends AbstractCashIntegrationTest {
    ...
    @Test
    void deposit_callsAccountsCredit() throws Exception {
        ...
        verify(accountsClient).credit(eq("alice"), eq(new BigDecimal("100.00")));
    }

    @Test
    void withdraw_callsAccountsDebit() throws Exception {
        when(accountsClient.debit(eq("alice"), any(BigDecimal.class)))
                .thenReturn(new AccountSnapshot("alice", "Alice", "Andreeva",
                        LocalDate.of(1990, 4, 12), new BigDecimal("900.00")));
        ...
        verify(accountsClient).debit(eq("alice"), eq(new BigDecimal("100.00")));
    }

    @Test
    void withdraw_propagatesInsufficientFunds() throws Exception {
        when(accountsClient.debit(eq("alice"), any(BigDecimal.class)))
